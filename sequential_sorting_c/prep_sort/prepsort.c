...
int getSlot(unsigned long val, const double max, const size_t len){
  return (int)((val/max)*len);
}



void setItem(void* arr, void* temp, const size_t item_size){
  int i;
  switch (item_size){
  case 1:
    *((char*)arr)=*(char*)temp;
  ...
  case 8:
    *((long*)arr)=*(long*)temp;
    break;
  default:
    fast_bytecopy((unsigned char*)arr, (unsigned char*)temp, item_size);
    break;
  }
}


void getVals(void* arr,int* table, const long base, const size_t len, const size_t item_size){
  int arrInd=0;
  const int nodeSize=item_size+sizeof(int);
  node* temp=NULL;
  for(int i=0;i<len;i++){
    if(table[i]){
      temp=(node*)(table[i]*nodeSize+base);
      while(temp->index){
	setItem(arr+item_size*arrInd, temp->val, item_size);
...
void prepSort(void* arr, const size_t len, const size_t item_size, int ordered,
  ...
  for(int i =0;i<len;i++){
    ...
    slot=getSlot(val_fun(getItem(arr, i, item_size), item_size)-ulMin, rMax, effectiveLen);
    temp=(node*)(nodes+i*nodeSize);
    temp->index=table[slot];
    table[slot]=i+1;
    setItem(temp->val, getItem(arr, i, item_size), item_size);

  }
  getVals(arr, table, base, effectiveLen, item_size);
  f.sort_fun_generic(arr, len, item_size , compar_fun); 
  free(table);
  free(nodes);
}
